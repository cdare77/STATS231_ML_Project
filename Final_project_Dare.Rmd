...
We only provide the performance plots of random forest and boosted trees for two of our stocks, AMD and NVDA, since the formatting libraries (i.e. `ggplotly` and `swipeR`) are not compatible with the labelling / facet structure of `autoplot()` in these instances by default — a more detailed knowledge of `facet_wrap()` and `facet_grid()` is likely needed to pass the above autoplots into any sort of `ggplotly` object. Aside from the formatting technicalities, we notice that the best-performing random trees models require a larger number of randomly selected predictors, though the boosted trees models generally capture less variation of the outcome as the number of predictors grows.
