...
With the results of our grid search saved locally (for each model across all four stocks), we are able to visualize how the accuracy of our models changes as we vary the value of our hyper-parameters. 
...
autoplot(ridge_tune_NVDA) + ggtitle("Ridge Regression NVDA") + theme_dark()
```
```{r eval = TRUE, include=TRUE, warning=FALSE, fig.show="hold", out.width="50%"}
...
autoplot(knn_tune_AMD) + ggtitle("k-Nearest Neighbors AMD") + theme_dark()
