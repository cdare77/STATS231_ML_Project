...
Based on the two tables, we see that there is a clear choice for the two best performing models: linear regression and boosted trees. However, there do also seem to be some issues with linear regression in the case of the INTC stock: if our linear regression was able to perfectly match the training data, either the recipe is overfitting or the `lm` engine (responsible for our model's fit) is running into some computational errors. If overfitting is the case, then this will likely come into light when we apply the linear regression model to our training data; in either case, the performance across the other three stocks is enough to justify choosing the model for our testing data.
...
Although linear regression and boosted trees perform nearly the same on the training data, there is a clear victor between the two models when introduced to unseen data. This likely indicates that the boosted trees model was simply fitting the training data, while the linear regression correctly identified the underlying linear relationships between the predictors and the outcome. 
...
```{r eval = TRUE, include=TRUE, warning=FALSE}
######################
#  Set up data for dataframes for AMD stock for predicting and comparing
######################
# STORE ACTUAL CLOSING VALUES AND VOLUMES
a <- loadSymbols("AMD", from="2023-11-20", to="2024-01-30")
my_dates2 <- index(AMD)
AMD2 <- data.frame(AMD)
AMD2$AMD.Date <- my_dates2

# INITIAL TABLE TO START PREDICTING AFTER / ADDING ENTRIES TO
a <- loadSymbols("AMD", from="2023-11-20", to="2024-01-02")
my_dates <- index(AMD)
AMD <- data.frame(AMD)
AMD$AMD.Date <- my_dates

# Re-Add Lag variables
AMD$AMD.Close_L1 <- lag(AMD$AMD.Close, 1)
AMD$AMD.Close_L2 <- lag(AMD$AMD.Close, 2)
AMD$AMD.Close_L3 <- lag(AMD$AMD.Close, 3)
AMD$AMD.Close_L4 <- lag(AMD$AMD.Close, 4)
...
#######################
# Begin iteratively adding predictions using linear regression
#######################
# Our AMD and AMD2 should align on the initial N days
AMD$AMD.Actual <- AMD$AMD.Close
# buffer variable to store predictions in
pred <- AMD$AMD.Close
N <- length(my_dates)

for(i in 1:15){
  # Variables that will be added to new row in dataframe
  new_date <- as.Date(my_dates2[(N+i)])
  new_L1 <- pred[(N +i - 1)]
  new_L2 <- pred[(N +i - 2)]
  new_L3 <- pred[(N +i - 3)]
  new_L4 <- pred[(N +i - 4)]
  ...
  new_L9 <- pred[(N +i - 9)]
  new_L10 <- pred[(N +i- 10)]
  new_SMA_cl_1W <- mean(pred[(N + i-5):(N+i-1)])
  new_SMD_cl_1W <- sd(pred[(N +i -5):(N+i-1)])
  new_EMA_cl_1W <- exponential_moving_average_helper(pred[(N + i-5):(N +i -1)], 5, (1/3))
  new_EMA_cl_2W <- exponential_moving_average_helper(pred[(N + i-10):(N +i - 1)], 10, (2/11))
  new_EMA_cl_1M <- exponential_moving_average_helper(pred[(N + i-20):(N+i-1)], 20, (2/21))
  new_MACD <- new_EMA_cl_2W - new_EMA_cl_1M
  # pull the ACTUAL information from our additional AMD2 dataframe
  new_vol <- AMD2$AMD.Volume[(N+i)]
  actual_value <- AMD2$AMD.Close[N+i]

  # Add new row to our dataframe based on previous predicted values
  AMD[N + i, ] = c(0, 0, 0, 0, new_vol, 0, new_date,
                   new_L1, new_L2, new_L3, new_L4, new_L5, new_L6, new_L7, new_L8, new_L9, new_L10,
                   new_SMA_cl_1W, 0, 0, 0,
                   new_EMA_cl_1W, new_EMA_cl_2W, new_EMA_cl_1M, 0,
                   new_SMD_cl_1W, 0, 0, 0,
                   new_MACD, actual_value)

  # predict next row, store in buffer variable
  pred <- predict(linear_fit_AMD, new_data = AMD %>% select(-AMD.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  AMD$AMD.Close[N+i] <- pred[N+i]
}

# Store results of linear regression in a different buffer
linear_pred <- AMD$AMD.Close

#######################
# Begin iteratively adding predictions for Elastic Net
#######################

# buffer variable to store predictions in
pred <- (AMD %>% slice(1:N))$AMD.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(elastic_net_fit_AMD, new_data = AMD %>% select(-AMD.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  AMD$AMD.Close[N+i] <- pred[N+i]
}

elastic_net_pred <- AMD$AMD.Close

#######################
# Begin iteratively adding predictions for k-Nearest Neighbors
#######################
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(knn_fit_AMD, new_data = AMD %>% select(-AMD.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  AMD$AMD.Close[N+i] <- pred[N+i]
}

knn_pred <- AMD$AMD.Close

#######################
# Begin iteratively adding predictions for Random Forest
#######################
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(random_forest_fit_AMD, new_data = AMD %>% select(-AMD.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  AMD$AMD.Close[N+i] <- pred[N+i]
}

random_forest <- AMD$AMD.Close

#######################
# Begin iteratively adding predictions for Boosted Trees
#######################
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(boosted_trees_fit_AMD, new_data = AMD %>% select(-AMD.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  AMD$AMD.Close[N+i] <- pred[N+i]
}

boosted_trees <- AMD$AMD.Close


# re-add results of linear regression from buffer into new variable
AMD$AMD.Linear_Pred <- linear_pred
AMD$AMD.Elastic_Net_Pred <- elastic_net_pred
AMD$AMD.KNN_Pred <- knn_pred
AMD$AMD.Random_Forest_Pred <- random_forest
AMD$AMD.Boosted_Trees_Pred <- boosted_trees
```
...
```{r eval = TRUE, include=TRUE, warning=FALSE}
######################
#  Set up data for dataframes for NVDA stock for predicting and comparing
######################
# STORE ACTUAL CLOSING VALUES AND VOLUMES
a <- loadSymbols("NVDA", from="2023-11-20", to="2024-01-30")
my_dates2 <- index(NVDA)
NVDA2 <- data.frame(NVDA)
NVDA2$NVDA.Date <- my_dates2

# INITIAL TABLE TO START PREDICTING AFTER / ADDING ENTRIES TO
a <- loadSymbols("NVDA", from="2023-11-20", to="2024-01-02")
my_dates <- index(NVDA)
NVDA <- data.frame(NVDA)
NVDA$NVDA.Date <- my_dates
...
# Begin iteratively adding predictions using linear regression
#######################
# Our NVDA and NVDA2 should align on the initial N days
NVDA$NVDA.Actual <- NVDA$NVDA.Close
# buffer variable to store predictions in
pred <- NVDA$NVDA.Close
N <- length(my_dates)
...
for(i in 1:15){
  ...
  new_EMA_cl_1M <- exponential_moving_average_helper(pred[(N + i-20):(N+i-1)], 20, (2/21))
  new_MACD <- new_EMA_cl_2W - new_EMA_cl_1M
  # pull the ACTUAL information from our additional NVDA2 dataframe
  new_vol <- NVDA2$NVDA.Volume[(N+i)]
  actual_value <- NVDA2$NVDA.Close[N+i]

  # Add new row to our dataframe based on previous predicted values
  NVDA[N + i, ] = c(0, 0, 0, 0, new_vol, 0, new_date,
                   new_L1, new_L2, new_L3, new_L4, new_L5, new_L6, new_L7, new_L8, new_L9, new_L10,
                   new_SMA_cl_1W, 0, 0, 0,
                   new_EMA_cl_1W, new_EMA_cl_2W, new_EMA_cl_1M, 0,
                   new_SMD_cl_1W, 0, 0, 0,
  ...
  # predict next row, store in buffer variable
  pred <- predict(linear_fit_NVDA, new_data = NVDA %>% select(-NVDA.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  NVDA$NVDA.Close[N+i] <- pred[N+i]
}

# Store results of linear regression in a different buffer
linear_pred <- NVDA$NVDA.Close
...
# buffer variable to store predictions in
pred <- (NVDA %>% slice(1:N))$NVDA.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(elastic_net_fit_NVDA, new_data = NVDA %>% select(-NVDA.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  NVDA$NVDA.Close[N+i] <- pred[N+i]
}

elastic_net_pred <- NVDA$NVDA.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(knn_fit_NVDA, new_data = NVDA %>% select(-NVDA.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  NVDA$NVDA.Close[N+i] <- pred[N+i]
}

knn_pred <- NVDA$NVDA.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(random_forest_fit_NVDA, new_data = NVDA %>% select(-NVDA.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  NVDA$NVDA.Close[N+i] <- pred[N+i]
}

random_forest <- NVDA$NVDA.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(boosted_trees_fit_NVDA, new_data = NVDA %>% select(-NVDA.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  NVDA$NVDA.Close[N+i] <- pred[N+i]
}

boosted_trees <- NVDA$NVDA.Close


# re-add results of linear regression from buffer into new variable
NVDA$NVDA.Linear_Pred <- linear_pred
NVDA$NVDA.Elastic_Net_Pred <- elastic_net_pred
NVDA$NVDA.KNN_Pred <- knn_pred
NVDA$NVDA.Random_Forest_Pred <- random_forest
NVDA$NVDA.Boosted_Trees_Pred <- boosted_trees
```


```{r eval = TRUE, include=TRUE, warning=FALSE}
######################
#  Set up data for dataframes for INTC stock for predicting and comparing
######################
# STORE ACTUAL CLOSING VALUES AND VOLUMES
a <- loadSymbols("INTC", from="2023-11-20", to="2024-01-30")
my_dates2 <- index(INTC)
INTC2 <- data.frame(INTC)
INTC2$INTC.Date <- my_dates2

# INITIAL TABLE TO START PREDICTING AFTER / ADDING ENTRIES TO
a <- loadSymbols("INTC", from="2023-11-20", to="2024-01-02")
my_dates <- index(INTC)
INTC <- data.frame(INTC)
INTC$INTC.Date <- my_dates
...
# Begin iteratively adding predictions using linear regression
#######################
# Our INTC and INTC2 should align on the initial N days
INTC$INTC.Actual <- INTC$INTC.Close
# buffer variable to store predictions in
pred <- INTC$INTC.Close
N <- length(my_dates)
...
for(i in 1:15){
  ...
  new_EMA_cl_1M <- exponential_moving_average_helper(pred[(N + i-20):(N+i-1)], 20, (2/21))
  new_MACD <- new_EMA_cl_2W - new_EMA_cl_1M
  # pull the ACTUAL information from our additional INTC2 dataframe
  new_vol <- INTC2$INTC.Volume[(N+i)]
  actual_value <- INTC2$INTC.Close[N+i]

  # Add new row to our dataframe based on previous predicted values
  INTC[N + i, ] = c(0, 0, 0, 0, new_vol, 0, new_date,
                   new_L1, new_L2, new_L3, new_L4, new_L5, new_L6, new_L7, new_L8, new_L9, new_L10,
                   new_SMA_cl_1W, 0, 0, 0,
                   new_EMA_cl_1W, new_EMA_cl_2W, new_EMA_cl_1M, 0,
                   new_SMD_cl_1W, 0, 0, 0,
  ...
  # predict next row, store in buffer variable
  pred <- predict(linear_fit_INTC, new_data = INTC %>% select(-INTC.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  INTC$INTC.Close[N+i] <- pred[N+i]
}

# Store results of linear regression in a different buffer
linear_pred <- INTC$INTC.Close
...
# buffer variable to store predictions in
pred <- (INTC %>% slice(1:N))$INTC.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(elastic_net_fit_INTC, new_data = INTC %>% select(-INTC.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  INTC$INTC.Close[N+i] <- pred[N+i]
}

elastic_net_pred <- INTC$INTC.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(knn_fit_INTC, new_data = INTC %>% select(-INTC.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  INTC$INTC.Close[N+i] <- pred[N+i]
}

knn_pred <- INTC$INTC.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(random_forest_fit_INTC, new_data = INTC %>% select(-INTC.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  INTC$INTC.Close[N+i] <- pred[N+i]
}

random_forest <- INTC$INTC.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(boosted_trees_fit_INTC, new_data = INTC %>% select(-INTC.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  INTC$INTC.Close[N+i] <- pred[N+i]
}

boosted_trees <- INTC$INTC.Close


# re-add results of linear regression from buffer into new variable
INTC$INTC.Linear_Pred <- linear_pred
INTC$INTC.Elastic_Net_Pred <- elastic_net_pred
INTC$INTC.KNN_Pred <- knn_pred
INTC$INTC.Random_Forest_Pred <- random_forest
INTC$INTC.Boosted_Trees_Pred <- boosted_trees
```
...
```{r eval = TRUE, include=TRUE, warning=FALSE}
######################
#  Set up data for dataframes for TSM stock for predicting and comparing
######################
# STORE ACTUAL CLOSING VALUES AND VOLUMES
a <- loadSymbols("TSM", from="2023-11-20", to="2024-01-30")
my_dates2 <- index(TSM)
TSM2 <- data.frame(TSM)
TSM2$TSM.Date <- my_dates2

# INITIAL TABLE TO START PREDICTING AFTER / ADDING ENTRIES TO
a <- loadSymbols("TSM", from="2023-11-20", to="2024-01-02")
my_dates <- index(TSM)
TSM <- data.frame(TSM)
TSM$TSM.Date <- my_dates
...
# Begin iteratively adding predictions using linear regression
#######################
# Our TSM and TSM2 should align on the initial N days
TSM$TSM.Actual <- TSM$TSM.Close
# buffer variable to store predictions in
pred <- TSM$TSM.Close
N <- length(my_dates)
...
for(i in 1:15){
  ...
  new_EMA_cl_1M <- exponential_moving_average_helper(pred[(N + i-20):(N+i-1)], 20, (2/21))
  new_MACD <- new_EMA_cl_2W - new_EMA_cl_1M
  # pull the ACTUAL information from our additional TSM2 dataframe
  new_vol <- TSM2$TSM.Volume[(N+i)]
  actual_value <- TSM2$TSM.Close[N+i]

  # Add new row to our dataframe based on previous predicted values
  TSM[N + i, ] = c(0, 0, 0, 0, new_vol, 0, new_date,
                   new_L1, new_L2, new_L3, new_L4, new_L5, new_L6, new_L7, new_L8, new_L9, new_L10,
                   new_SMA_cl_1W, 0, 0, 0,
                   new_EMA_cl_1W, new_EMA_cl_2W, new_EMA_cl_1M, 0,
                   new_SMD_cl_1W, 0, 0, 0,
  ...
  # predict next row, store in buffer variable
  pred <- predict(linear_fit_TSM, new_data = TSM %>% select(-TSM.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  TSM$TSM.Close[N+i] <- pred[N+i]
}

# Store results of linear regression in a different buffer
linear_pred <- TSM$TSM.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(elastic_net_fit_TSM, new_data = TSM %>% select(-TSM.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  TSM$TSM.Close[N+i] <- pred[N+i]
}

elastic_net_pred <- TSM$TSM.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(knn_fit_TSM, new_data = TSM %>% select(-TSM.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  TSM$TSM.Close[N+i] <- pred[N+i]
}

knn_pred <- TSM$TSM.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(random_forest_fit_TSM, new_data = TSM %>% select(-TSM.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  TSM$TSM.Close[N+i] <- pred[N+i]
}

random_forest <- TSM$TSM.Close
...
for(i in 1:15){
  ...
  # predict next row, store in buffer variable
  pred <- predict(boosted_trees_fit_TSM, new_data = TSM %>% select(-TSM.Close))$.pred
  # update the closing price of the N+i-th day to our prediction
  TSM$TSM.Close[N+i] <- pred[N+i]
}

boosted_trees <- TSM$TSM.Close


# re-add results of linear regression from buffer into new variable
TSM$TSM.Linear_Pred <- linear_pred
TSM$TSM.Elastic_Net_Pred <- elastic_net_pred
TSM$TSM.KNN_Pred <- knn_pred
TSM$TSM.Random_Forest_Pred <- random_forest
TSM$TSM.Boosted_Trees_Pred <- boosted_trees
```
...
ggplot(data = AMD, aes(x=AMD.Date)) +
  geom_line(aes(y = AMD.Linear_Pred, color = 'Linear Regression Prediction')) +
  geom_line(aes(y = AMD.Elastic_Net_Pred, color = 'Elastic Net Prediction')) +
  geom_line(aes(y = AMD.KNN_Pred, color = 'k-Nearest Neighbors Prediction')) +
  geom_line(aes(y = AMD.Boosted_Trees_Pred, color = 'Boosted-Trees Prediction')) +
  geom_line(aes(y = AMD.Random_Forest_Pred, color = 'Random Forest Prediction')) +
  geom_line(aes(y = AMD.Actual, color = 'Actual Closing Value')) +
  ylab('USD') +
  scale_color_manual(values = c(
  ...
  xlab('Date') +
  ggtitle("Predicted Close Price of AMD") +
  scale_y_continuous(labels = label_comma()) +
  theme_dark()

# plot different outcomes for NVDA
ggplot(data = NVDA, aes(x=NVDA.Date)) +
  geom_line(aes(y = NVDA.Linear_Pred, color = 'Linear Regression Prediction')) +
  geom_line(aes(y = NVDA.Elastic_Net_Pred, color = 'Elastic Net Prediction')) +
  geom_line(aes(y = NVDA.KNN_Pred, color = 'k-Nearest Neighbors Prediction')) +
  geom_line(aes(y = NVDA.Boosted_Trees_Pred, color = 'Boosted-Trees Prediction')) +
  geom_line(aes(y = NVDA.Random_Forest_Pred, color = 'Random Forest Prediction')) +
  geom_line(aes(y = NVDA.Actual, color = 'Actual Closing Value')) +
  ylab('USD') +
  scale_color_manual(values = c(
    'Boosted-Trees Prediction' = 'coral1',
    'k-Nearest Neighbors Prediction' = 'brown1',
  ...
  xlab('Date') +
  ggtitle("Predicted Close Price of NVDA") +
  scale_y_continuous(labels = label_comma()) +
  theme_dark()

# plot different outcomes for INTC
ggplot(data = INTC, aes(x=INTC.Date)) +
  geom_line(aes(y = INTC.Linear_Pred, color = 'Linear Regression Prediction')) +
  geom_line(aes(y = INTC.Elastic_Net_Pred, color = 'Elastic Net Prediction')) +
  geom_line(aes(y = INTC.KNN_Pred, color = 'k-Nearest Neighbors Prediction')) +
  geom_line(aes(y = INTC.Boosted_Trees_Pred, color = 'Boosted-Trees Prediction')) +
  geom_line(aes(y = INTC.Random_Forest_Pred, color = 'Random Forest Prediction')) +
  geom_line(aes(y = INTC.Actual, color = 'Actual Closing Value')) +
  ylab('USD') +
  scale_color_manual(values = c(
    'Boosted-Trees Prediction' = 'coral1',
    'k-Nearest Neighbors Prediction' = 'brown1',
  ...
  xlab('Date') +
  ggtitle("Predicted Close Price of INTC") +
  scale_y_continuous(labels = label_comma()) +
  theme_dark()
...
ggplot(data = TSM, aes(x=TSM.Date)) +
  geom_line(aes(y = TSM.Linear_Pred, color = 'Linear Regression Prediction')) +
  geom_line(aes(y = TSM.Elastic_Net_Pred, color = 'Elastic Net Prediction')) +
  geom_line(aes(y = TSM.KNN_Pred, color = 'k-Nearest Neighbors Prediction')) +
  geom_line(aes(y = TSM.Boosted_Trees_Pred, color = 'Boosted-Trees Prediction')) +
  geom_line(aes(y = TSM.Random_Forest_Pred, color = 'Random Forest Prediction')) +
  geom_line(aes(y = TSM.Actual, color = 'Actual Closing Value')) +
  ylab('USD') +
  scale_color_manual(values = c(
    'Boosted-Trees Prediction' = 'coral1',
    'k-Nearest Neighbors Prediction' = 'brown1',
  ...
  xlab('Date') +
  ggtitle("Predicted Close Price of TSM") +
  scale_y_continuous(labels = label_comma()) +
  theme_dark()
```
...
In a certain sense, stock market data is one of the most difficult things to completely predict — societal influences, supply chain issues, and even international politics can cast ripples which are difficult for any model to predict. On the other hand, our analysis shows that Occam's razor holds its merit even within the discipline of statistical learning: the model that is simplest to describe will often be the correct fit. After exploring a bit of the theory surrounding time-series, however, this outcome is not entirely unexpected: one of the most popular models for predicting stock-market behavior is known as ARIMA (Auto-Regression Integrated Moving Average). As the name suggests, this is a well-tuned regression model trained on its lag variables and moving averages, together with an "integrating" / differencing step which  handles seasonal or non-stationary data. Ultimately, seasonality and trends in the data were not factors taken into account for this analysis, and thus will likely 
